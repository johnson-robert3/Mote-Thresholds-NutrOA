...
# Biomass cleaning and processing
...
# Carbohydrate cleaning and processing
rhiz_carbs <- rhizome_carbs_raw %>%
   ...
   # add vial info
   left_join(rhizome_carb_ids %>% select(vial_number, vial_id, sample_id, sample_notes = notes), by = "vial_number") %>%
   select(-vial_number, -lab_sample_num, -vial_id, -sample_notes) %>%
   relocate(sample_id) %>%
...
meadow_rhiz_carbs <- rhizome_carbs_raw %>%  # from carbohydrate data file above
   # discrepancy in glucose for vial 27, glucose for 'main' sample was <0.2 but dupe was 0.4; replace 'main' sample glucose with 0.2, instead of flagging it below DL
   mutate(glucose_percent = replace(glucose_percent, client_vial_num == 27, 0.2)) %>%
   filter_out(str_detect(lab_sample_num, pattern="dup")) %>%  # analytical dupes look good
   rename(vial_number = client_vial_num, analysis_notes = notes) %>%
   # add vial info
   left_join(rhizome_carb_ids %>% select(vial_number, vial_id, sample_id, sample_notes = notes), by = "vial_number") %>%
   select(-vial_number, -lab_sample_num, -vial_id) %>%
   relocate(sample_id) %>%
